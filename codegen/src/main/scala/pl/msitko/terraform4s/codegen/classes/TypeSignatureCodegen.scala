...
object TypeSignatureCodegen {
  ...
  def fromHCLType(v: HCLType, ctx: CodegenContext): Type = v match {
    ...
    case HCLList(t) => Type.Apply(Type.Name("List"), List(fromHCLType(t, ctx)))
    case HCLMap(t)  => Type.Apply(Type.Name("Map"), List(Type.Name("String"), fromHCLType(t, ctx)))
    case HCLSet(t)  => Type.Apply(Type.Name("Set"), List(fromHCLType(t, ctx)))
    case obj: HCLObject =>
